...
fun FilesTopBar(
    ...
    onSearchClose: () -> Unit = {},
    onSortClick: () -> Unit, // Botón de ordenar siempre disponible
    onPremiumClick: () -> Unit
) {
    ...
    TopAppBar(
        ...
        actions = {
            if (selectionMode) {
                ...
                IconButton(onClick = onPremiumClick) { Icon(Icons.Default.Diamond, contentDescription = "Premium", tint = Color(0xFF00BCD4)) }
                IconButton(onClick = onSearchTrigger) { Icon(Icons.Default.Search, contentDescription = "Buscar") }
                IconButton(onClick = onSortClick) { Icon(Icons.AutoMirrored.Filled.Sort, contentDescription = "Ordenar") } // Siempre visible
                IconButton(onClick = onToggleView) {
                    Icon(if (isGrid) Icons.AutoMirrored.Filled.FormatListBulleted else Icons.Default.Apps, contentDescription = "Vista")
                }
            } else {
                // En modo búsqueda también permitimos ordenar y limpiar texto
                if (searchQuery.isNotEmpty()) {
                    IconButton(onClick = { onSearchQueryChange("") }) {
                        Icon(Icons.Default.Close, contentDescription = "Limpiar")
                    }
                }
                IconButton(onClick = onSortClick) { Icon(Icons.AutoMirrored.Filled.Sort, contentDescription = "Ordenar") }
            }
        }
    )
}